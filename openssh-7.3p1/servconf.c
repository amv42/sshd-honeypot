...
typedef enum {
	...
	/* Portable-specific options */
	sUsePAM,
	/* Standard Options + sshd_honey edit - first six entries*/
	msHost, msPort, msUsername, sshdVersion, tcpForwardingHost, tcpForwardingPort, sPort, sHostKeyFile, sServerKeyBits, sLoginGraceTime,
...
/* sshd_honey edit */
	{ "mshost", msHost, SSHCFG_GLOBAL },
	{ "msport", msPort, SSHCFG_GLOBAL },
	{ "msusername", msUsername, SSHCFG_GLOBAL },
    { "sshdversion", sshdVersion, SSHCFG_GLOBAL },
	{ "tcpforwardinghost", tcpForwardingHost, SSHCFG_GLOBAL },
	{ "tcpforwardingport", tcpForwardingPort, SSHCFG_GLOBAL },
/* sshd_honey end
...
/* sshd-honeypot: Add sshd_config options */
	case msHost:
		//debug("test");
        arg = strdelim(&cp);
        p = hpdelim(&arg);
        p = cleanhostname(p);
        sprintf(sshd_honey_options.ip, "%s", p);
        //fatal("%s mshost;", sshd_honey_options.ip);
        break;

	case msPort:
		arg = strdelim(&cp);
            sshd_honey_options.port = a2port(arg);
        //fatal("%d msport;", sshd_honey_options.port);
        break;

	case msUsername:
		arg = strdelim(&cp);
			if (arg == NULL || *arg == '\0')
				fatal("%s line %d: missing msUsername",
					  filename, linenum);
		sprintf(sshd_honey_options.username, "%s", arg);
        //fatal("%s msusername;", sshd_honey_options.username);
        break;

    case sshdVersion:
         arg = strdelim(&cp);
         sprintf(sshd_honey_options.server_version, "%s", arg);
         //fatal("%s sshversion", SSH_VERSION);
         break;

    case tcpForwardingHost:
		 arg = strdelim(&cp);
      	 p = hpdelim(&arg);
         p = cleanhostname(p);
         sprintf(sshd_honey_options.tcpForwardingHost, "%s", p);
         //fatal("%d tcpForwardingHost;", sshd_honey_options.port);
         break;

    case tcpForwardingPort:
         arg = strdelim(&cp);
            sshd_honey_options.tcpForwardingPort = a2port(arg);
         //fatal("%d tcpForwardingPort;", sshd_honey_options.port);
         break;
/* sshd-honeypot: end */
